...
addon = xbmcaddon.Addon('plugin.video.fmc.hd')
...
favoritosdb = os.path.join(profile, 'favoritos.db')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.fmc.hd-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(FMC.HD, erro de codigo - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(FMC.HD, erro de servidor. - "+str(e.reason)+",10000,"+icon+")")
def SKindex():
    addDir('[B][COLOR blue]| Favoritos[/COLOR][/B]','[B][COLOR blue]| Favoritos[/COLOR][/B]',4,'https://i.imgur.com/3sHwOwL.png',FANART,'[COLOR white][B]FMC[COLOR lime].[COLOR white]HD[/B][/COLOR]','','','')
    getData(Base,FANART)
...
def getSources():
        try:
            if os.path.exists(favoritos) == True:
                addDir('[B][COLOR blue][ Favoritos ][/COLOR][/B]','[B][COLOR blue][ Favoritos ][/COLOR][/B]',4,'https://i.imgur.com/3sHwOwL.png' , os.path.join(home, 'fanart.jpg'),FANART,'','','','')
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(FMC.HD, nova fonte adicionada.,5000,"+icon+")")
        if not url is None:
...
def getSearchData(url,icon, data=None):
    ...
    if keyboard.isConfirmed():
        ...
        xbmcgui.Dialog().ok('[COLOR white][B]FMC[COLOR lime].[COLOR white]HD[/B][/COLOR]', '[COLOR red][B]ATENÇÃO:[/B] [COLOR white]Pesquisas em branco não são autorizadas.[/COLOR]')
        quit()
    fanart=''
    dontLink=False
    os.path.join(home, 'resources', 'fanart.jpg')
...
def playmediawithproxy(media_url, name, iconImage,proxyip,port, proxyuser=None, proxypass=None): #jairox
    ...
    if media_url==None or media_url=='':
        xbmc.executebuiltin("XBMC.Notification(fmc.hd,Unable to play empty Url,5000,"+icon+")")
        return
        ...
        try:
            while player.is_active:
                ...
                if player.urlplayed==False and time.time()-beforestart>12:
                    print 'failed!!!'
                    xbmc.executebuiltin("XBMC.Notification(FMC.HD,Unable to play check proxy,5000,"+icon+")")
                    break
                #xbmc.sleep(1000)
        except: pass
...
def getFavoritos():
        if os.path.exists(favoritos):
            ...
            for i in items:
                ...
                addDir('[COLOR red][B][ Sem  Favoritos adicionados ][/B][/COLOR]','','','','','','','','','fav')
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(FMC.HD,Urlresolver nao suporta este dominio. - ,5000)")
        resolver=url
    return resolver
...
def download_file(name, url):
        ...
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('fmc.hd','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('fmc.hd', 'Do you want to add this file as a source?')
        if ret:
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('[B][COLOR deepskyblue]Remover FMC Favorito[/COLOR][/B]','XBMC.RunPlugin(%s?mode=6&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
            if not name in FAV:
                contextMenu.append(('[B][COLOR deepskyblue]Adicionar a FMC Favoritos[/COLOR][/B]','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#                print 'setting as folder in link'
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('[B][COLOR deepskyblue]Remover FMC Favorito[/COLOR][/B]','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('[B][COLOR deepskyblue]Adicionar a FMC Favoritos[/COLOR][/B]','XBMC.RunPlugin(%s)' %fav_params))                                                                                
            liz.addContextMenuItems(contextMenu)
        try:
...
#            ln+='</item>'
        ...
        if not (regexs and 'notplayable' in regexs and not url):        
            if url:
                ...
                xbmc.executebuiltin("XBMC.Notification(FMC.HD,Falha ao extrair regex. - "+"this"+",4000,"+icon+")")
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(FMC.HD,Instala o [COLOR yellow]  Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(fmc.hd,Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(fmc.hd,Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
    
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(fmc.hd,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
