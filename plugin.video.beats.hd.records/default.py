...
if os.path.exists(profile):
    ...
    #xbmcaddon.Addon(id='plugin.video.beats.hd.records').openSettings()
    os.mkdir(TRANSLATEPATH(addon.getAddonInfo('profile')))

def addon_log(string, level=xbmc.LOGDEBUG):
    if debug == 'true':
        xbmc.log("[plugin.video.beats.hd.records-{0}]: {1}".format(addon_version, string), LOGINFO)
    else:
        xbmc.log("[plugin.video.beats.hd.records-{0}]: {1}".format(addon_version, string), level)
...
def addDir(name, url, mode, iconimage, fanart, description, genre, date, credits, showcontext=False, regexs=None, reg_url=None, allinfo={}):
    ...
    if showcontext:
        ...
        if showcontext == 'source':
            ...
            contextMenu.append(('Remove from beats.hd.records Favorites', 'RunPlugin(%s?mode=6&name=%s)'
        ...
        if name not in FAV:
            contextMenu.append(('Add to beats.hd.records Favorites', 'RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def addLink(url, name, iconimage, fanart, description, genre, date, showcontext, playlist, regexs, total, setCookie="", allinfo={}):
    ...
    if showcontext:
        if showcontext == 'fav':
            contextMenu.append(
                ('Remove from beats.hd.records Favorites', 'RunPlugin(%s?mode=6&name=%s)'
                 % (sys.argv[0], urllib_parse.quote_plus(name)))
            )
            ...
            contextMenu.append(('Add to beats.hd.records Favorites', 'RunPlugin(%s)' % fav_params))
        liz.addContextMenuItems(contextMenu)
    try:
